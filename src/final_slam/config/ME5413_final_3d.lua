...
options = {
  ...
  publish_frame_projected_to_2d = true, -- 位姿投影到2D，防止z漂移影响导航
...
TRAJECTORY_BUILDER_3D.max_range = 15.0 -- 30 -> 15 减少点云处理量
-- 3D模式下用 TRAJECTORY_BUILDER_3D 的 z 裁剪
TRAJECTORY_BUILDER_3D.high_resolution_adaptive_voxel_filter.min_num_points = 150 -- 默认200，减少点数
TRAJECTORY_BUILDER_3D.low_resolution_adaptive_voxel_filter.min_num_points = 100  -- 默认200
TRAJECTORY_BUILDER_3D.submaps.num_range_data = 300 -- 500 -> 300 减小submap数据量
TRAJECTORY_BUILDER_3D.voxel_filter_size = 0.25 -- 0.15 -> 0.25 进一步下采样
TRAJECTORY_BUILDER_3D.use_online_correlative_scan_matching = false -- 关掉最耗CPU的暴力匹配
TRAJECTORY_BUILDER_3D.ceres_scan_matcher.rotation_weight = 0.1

MAP_BUILDER.num_background_threads = 10
POSE_GRAPH.optimization_problem.huber_scale = 5e2
POSE_GRAPH.optimize_every_n_nodes = 1500 -- 1000 -> 1500 降低全局优化频率
POSE_GRAPH.constraint_builder.sampling_ratio = 0.03
POSE_GRAPH.optimization_problem.ceres_solver_options.max_num_iterations = 16
POSE_GRAPH.constraint_builder.min_score = 0.95
POSE_GRAPH.constraint_builder.global_localization_min_score = 0.95
